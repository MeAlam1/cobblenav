...
class CoordinatesCollector : GeneralConditionCollector() {
	...
	override fun collectValues(detail: SpawnDetail, condition: SpawningCondition<*>, player: ServerPlayer): List<MutableComponent>? {
		...
		formatValueRange(condition.minX, condition.maxX)?.let { values.add(label("spawn_data.coordinates.x", it)) }
		formatValueRange(condition.minZ, condition.maxZ)?.let { values.add(label("spawn_data.coordinates.z", it)) }
