...
object BucketSpeciesFeatureProvider :
	...
	@JvmStatic
	fun apply(pokemon: Pokemon, bucket: String) {
		StringSpeciesFeature(keys.first(), bucket).apply(pokemon)
	}

	override fun fromString(value: String?): StringSpeciesFeature? = value?.lowercase()?.let { StringSpeciesFeature(keys.first(), it) }

	override fun examples(): Collection<String> = emptyList()

	override fun invoke(buffer: RegistryFriendlyByteBuf, name: String): StringSpeciesFeature? = if (name in keys) {
	...
	override fun invoke(pokemon: Pokemon): StringSpeciesFeature = get(pokemon) ?: StringSpeciesFeature(keys.first(), "")
