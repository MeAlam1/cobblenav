...
import com.metacontent.cobblenav.utils.extensions.*
...
) : SoundlessWidget(x, y, width, height, label("weather")) {
	companion object {
	...
	override fun renderWidget(guiGraphics: GuiGraphics, i: Int, j: Int, f: Float) {
		...
		level?.let {
			...
			if (adjustedTime in 23000..24000 || adjustedTime in 0..13702) {
				if (cloudOpacity < MAX_CLOUD_OPACITY) {
					cloudOpacity =
			...
			if (adjustedTime in 11834..24000 || adjustedTime in 0..167) {
				...
				if (starsOpacity < MAX_STARS_OPACITY) {
					starsOpacity =
