...
import com.metacontent.cobblenav.client.gui.screen.ConfigScreen
...
class ConfigVariableList(
	...
	class Entry(
		...
		private val label: Component = guiLang("$idPrefix.option.${option.name}")
		...
		private fun buildResetButton(): Button = Button.builder(lang("reset")) {
			option.reset()
			refreshWidgetValue()
			updateResetState()
		}
